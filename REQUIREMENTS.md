...
- [x] Teacher can create and edit routines in the web interface
- [x] Each routine has a title and text content (poems, songs, instructions), and also a sound file (mp3/wav) which can be uploaded by the teacher or generated by the system using any TTS library installed
- [x] Text-to-speech conversion with adjustable speed
- [x] Support for multiple voices/languages
- [x] Visual text display synchronized with speech
- [x] Pause, resume, and replay controls
- [x] Pre-built routines library (e.g., hand-warming exercise)
- [x] Teacher can categorize routines (warm-up, transition, cooldown)
- [x] **Multi-Engine TTS Support**:
  - [x] pyttsx3: System TTS (offline, works everywhere)
  - [x] edge-tts: High-quality online voices (requires internet)
  - [x] gTTS: Google TTS (requires internet)
  - [x] Teacher can select preferred engine per routine
  - [x] Configurable speed (0.5x - 2.0x)
  - [x] Language support (en, ro, and others)
- [x] **USB Presenter Support**:
  - [x] Compatible with standard USB wireless presenters (Logitech, Kensington, etc.)
  - [x] Configurable button mappings
  - [x] Next/Previous/Play/Pause/Stop buttons supported
  - [x] Auto-detects presenter when plugged in
...
- TTS engines: pyttsx3, edge-tts, gTTS (teacher selects preferred)
- Audio file upload overrides TTS
- Line-by-line playback with synchronized highlighting
...
- **Wireless Presenter Support**:
  ...
  - Uses `evdev` library to read USB input events (Linux only)
  - No drivers needed - works with any standard presenter
  - Graceful fallback when evdev not available (Windows/Mac)
...
- [x] **Auto-Discovery**: Plugins automatically discovered without manual INSTALLED_APPS registration
  - [x] `discover_plugin_apps()` function scans plugins directory at startup
  - [x] Automatically adds plugins to Django's `INSTALLED_APPS`
  - [x] No manual configuration needed for new plugins
...
class Plugin(PluginBase):
    ...
    def register(self):
        ...
        self.register_admin_menu('Weather', '/plugins/acme/weather/')
...
- **Auto-Discovery**: Plugins automatically discovered and registered
...
4. "I want to play classroom routines with text-to-speech"
5. "I want to control routines with a wireless presenter"
...
4. "I want to follow along with warm-up exercises"
...
- **TTS**: pyttsx3, edge-tts, gTTS (multiple engines) ✅
...
│   └── settings.py        # With auto-discovery
...
- `WS /ws/routines/` - WebSocket for routine playback sync
- `POST /plugins/edupi/routines/api/control/` - Routine playback control
- `GET /plugins/edupi/routines/api/status/` - Routine status
...
- ✅ **Auto-discovery from `plugins/` directory** - No manual INSTALLED_APPS registration
...
- ✅ Auto-discovery function `discover_plugin_apps()` in config/settings.py

**Auto-Discovery Implementation:**
- ✅ `discover_plugin_apps()` scans `plugins/*/*/` directories
- ✅ Automatically registers plugins with Django's `INSTALLED_APPS`
- ✅ No manual configuration needed for new plugins
- ✅ Duplicate detection and removal
- ✅ Maintains plugin load order
...
**User Story**: As a teacher, I want to define classroom routines that the system reads aloud so that students can follow along.
...
- ✅ Audio file upload (MP3/WAV) to override TTS
- ✅ Line-by-line playback with synchronized highlighting
- ✅ USB Presenter support:
  - Compatible with standard USB HID presenters
...
- ✅ Pre-built routines:
  ...
  - Finger Stretch
  - Deep Breathing
- ✅ Category management (Warm-up, Transition, Cooldown)
- ✅ Keyboard shortcuts (Space, Arrows, Escape)
- ✅ TTS preview with live testing

**Technical Implementation:**
- TTS Manager with multi-engine support
- Audio Player using pygame
- Routine Player with line-by-line control
- Presenter Handler with evdev support (Linux)
- WebSocket Consumer for real-time sync
- WebSocket routing added to ASGI config

**Access URLs:**
- Dashboard: `/plugins/edupi/routines/`
- Player: `/plugins/edupi/routines/<id>/play/`
- API: `/plugins/edupi/routines/api/`
- WebSocket: `ws://localhost:8000/ws/routines/`
